...
class GeSHi
{
	...
	function error ()
	{
		if ($this->error) {
			...
			$msg = str_replace(array_keys($debug_tpl_vars), array_values($debug_tpl_vars),
			...
			while ($i < $length) {
					...
					if (is_array($delimiters)) {
						foreach ($delimiters as $open => $close) {
							...
							if ($matches[ $dk ][ $open ]['next_match'] < $next_match_pos) {
								...
								$matches[ $dk ][ $open ]['close_pos'] = strpos($code, $close,
									$matches[ $dk ][ $open ]['next_match'] + 1);
				...
				if (is_array($delim_copy[ $next_match_pointer['dk'] ])) {
					...
					while (TRUE) {
						...
						if ($code[ $i ] == $next_match_pointer['open'][0] && ($next_match_pointer['open_strlen'] == 1 || substr($code,
									$i, $next_match_pointer['open_strlen']) == $next_match_pointer['open'])
						) {
		...
		for ($key = 0; $key < $num_parts; ++$key) {
			...
			if ($highlight_part) {
				...
				for ($i = 0; $i < $length; ++$i) {
					...
					if (isset($is_string_starter[ $char ])) {
						...
						if (is_array($is_string_starter[ $char ])) {
							...
							foreach ($is_string_starter[ $char ] as $testchar) {
								if ($testchar === substr($part, $i,
										strlen($testchar)) && strlen($testchar) > strlen($char_new)
								) {
									$char_new = $testchar;
									$string_started = TRUE;
								}
					...
					if ($string_started && ($i != $next_comment_regexp_pos)) {
						...
						do {
									...
									foreach ($this->language_data['ESCAPE_REGEXP'] as $escape_key => $regexp) {
										...
										elseif (//This is to allow use of the offset parameter in preg_match and stay as compatible with older PHP versions as possible
											(GESHI_PHP_PRE_433 && preg_match($regexp, substr($part, $start), $match,
													...
													$part, $match, PREG_OFFSET_CAPTURE, $start))
										) {
											$match_i = $match[0][1];
											if (GESHI_PHP_PRE_433) {
												$match_i += $start;
							...
							elseif ($next_escape_regexp_pos < $length && $next_escape_regexp_pos < $close_pos) {
								...
								$string .= $this->hsc(substr($part, $start,
										$close_pos - $start + $char_len)) . '</span>';
					...
					elseif ($this->lexic_permissions['STRINGS'] && $hq && $hq[0] == $char && substr($part, $i,
						...
						if ($this->lexic_permissions['ESCAPE_CHAR'] && $this->language_data['ESCAPE_CHAR']) {
							...
							while ($es_pos = strpos($string, $this->language_data['ESCAPE_CHAR'], $start)) {
								...
								if ($c) {
									$new_string .= "<span$escape_char_attributes>" . str_repeat($escaped_escape_char,
											$c) . '</span>';
									$start = $es_pos + $c;
								}
								else {
						...
						if (!$COMMENT_MATCHED) {
							...
							if ($i == $next_comment_multi_pos) {
								...
								// Short-cut through all the multiline code
								$rest_of_comment = $this->hsc(substr($part, $i + $open_strlen,
									$close_pos - $i - $open_strlen + $close_strlen));
							...
							if (!empty($this->language_data['COMMENT_SINGLE']) && $next_comment_single_pos < $i) {
								...
								foreach ($this->language_data['COMMENT_SINGLE'] as $comment_key => $comment_mark) {
									...
									elseif (// case sensitive comments
										($this->language_data['CASE_SENSITIVE'][ GESHI_COMMENTS ] && ($match_i = stripos($part,
												$comment_mark, $i)) !== FALSE) || // non case sensitive
										(!$this->language_data['CASE_SENSITIVE'][ GESHI_COMMENTS ] && (($match_i = strpos($part,
													$comment_mark, $i)) !== FALSE))
									) {
	...
	function handle_keyword_replace ($match)
	{
		...
		if ($this->keyword_links) {
			...
			if (isset($this->language_data['URLS'][ $k ]) && $this->language_data['URLS'][ $k ] != '') {
				...
				if (!$this->language_data['CASE_SENSITIVE'][ $k ] && strpos($this->language_data['URLS'][ $k ],
						'{FNAME}') !== FALSE
				) {
	...
	function handle_regexps_callback ($matches)
	{
		...
		return ' style="' . call_user_func($this->language_data['STYLES']['REGEXPS'][ $this->_rx_key ],
				$matches[1]) . '"' . $matches[1] . '|>';
	}
	...
	function parse_non_string_part ($stuff_to_parse)
	{
			...
			if ($this->lexic_permissions['REGEXPS'][ $key ]) {
				if (is_array($regexp)) {
					if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
						...
						$stuff_to_parse = preg_replace_callback("/" . $regexp[ GESHI_SEARCH ] . "/{$regexp[GESHI_MODIFIERS]}",
							array (
								$this,
		...
		if ($this->lexic_permissions['NUMBERS'] && preg_match($this->language_data['PARSER_CONTROL']['NUMBERS']['PRECHECK_RX'],
				$stuff_to_parse)
		) {
			$numbers_found = TRUE;
			...
			foreach ($this->language_data['OBJECT_SPLITTERS'] as $key => $splitter) {
				if (FALSE !== strpos($stuff_to_parse, $splitter)) {
					...
					$stuff_to_parse = preg_replace("/($oolang_before)(" . preg_quote($this->language_data['OBJECT_SPLITTERS'][ $key ],
							'/') . ")($oolang_spaces)($oolang_after)/", "\\1\\2\\3<|$attributes>\\4|>",
						$stuff_to_parse);
				}
			}
		}
		...
		if ($this->lexic_permissions['SYMBOLS'] && !empty($this->language_data['SYMBOLS'])) {
			...
			for ($s_id = 0; $s_id < $n_symbols; ++$s_id) {
				...
				if ($this->language_data['MULTIPLE_SYMBOL_GROUPS']) {
					...
					preg_match_all("/" . $this->language_data['SYMBOL_SEARCH'] . "/", $symbol_match, $sym_match_syms,
						PREG_PATTERN_ORDER);
				...
				$stuff_to_parse = substr_replace($stuff_to_parse, $symbol_hl, $symbol_offset + $global_offset,
					$symbol_length);
			...
			if ($this->lexic_permissions['REGEXPS'][ $key ]) {
				if (is_callable($this->language_data['STYLES']['REGEXPS'][ $key ])) {
					...
					if (!$this->use_classes) {
						...
						if (is_array($this->language_data['REGEXPS'][ $key ]) && array_key_exists(GESHI_CLASS,
	...
	function merge_arrays ()
	{
		...
		for ($i = 0; $i < $narrays; $i++) {
			if (!is_array($arrays[ $i ])) {
				...
				trigger_error('Argument #' . ($i + 1) . ' is not an array - trying to merge array with scalar! Returning false!',
					E_USER_WARNING);
	...
	function get_stylesheet ($economy_mode = TRUE)
	{
		...
		foreach ($this->language_data['STYLES']['REGEXPS'] as $group => $styles) {
			if ($styles != '' && (!$economy_mode || (isset($this->lexic_permissions['REGEXPS'][ $group ]) && $this->lexic_permissions['REGEXPS'][ $group ]))) {
				if (is_array($this->language_data['REGEXPS'][ $group ]) && array_key_exists(GESHI_CLASS,
	...
	function optimize_regexp_list ($list, $regexp_delimiter = '/')
	{
				...
				if (isset($prev_keys[ $level ])) {
					...
					if ($char > 0) {
						...
						if ($char == strlen($prev_keys[ $level ])) {
							...
							if (in_array($new_key_part1[0], $regex_chars) || in_array($new_key_part2[0],
									$regex_chars)
							) {
		...
		$new_entry = $this->_optimize_regexp_list_tokens_to_string($tokens);
		if (GESHI_MAX_PCRE_SUBPATTERNS && $num_subpatterns + substr_count($new_entry,
